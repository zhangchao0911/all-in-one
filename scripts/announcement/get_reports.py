#!/usr/bin/env python3
"""查询 A 股研报列表 --mode all（按日期查全市场）或 --mode single（按股票代码查全部时期）"""
import os
import sys
import urllib.parse
...
def main():
    parser = ArgParser(description="查询 A 股研报列表（支持按日期或按股票代码查询）")
    parser.add_argument(
        "--mode",
        required=True,
        choices=["all", "single"],
    ...
    parser.add_argument("--stock-code", default=None, help="股票代码，含市场后缀，如 000001.SZ（mode=single 时必填）")
